...
credential_path = "/docker_api/apikey.json"
...
def transcribe_gcs(gcs_url):
    ...
    toexcel(response,gcs_url)


def toexcel(response,gcs_url):
    date = datetime.date.today().strftime("%Y-%m-%d")
    wb = Workbook()
    bn = date+'-1.xlsx'
    # 檢查檔案是否存在
    if os.path.isfile(bn):
        wb = load_workbook(bn)
        checkexcel(response,gcs_url,wb,bn)
    else:
        checkexcel(response,gcs_url,wb,bn)


def checkexcel(response,gcs_url,wb,bn):
    # date = '2020-02-09'
    date = datetime.date.today().strftime("%Y-%m-%d")
    with open(date+'.txt','a') as f:
        f.write(gcs_url+'\n')
...
print('end')
# list_blobs1(bucket, '2020-02-10')
# tostorage('2020-02-10.xlsx')